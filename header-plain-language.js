(() => {
  const dictionaries = {
    de: {
      ...
      storageTitle: "Deine Angaben bleiben auf diesem Gerät.",
      storageLead: "Vote4Gov verfolgt dich nicht. Nicht gesendete Vormerkungen bleiben nur hier und zählen nicht als öffentliche Stimme.",
      more: "Mehr erfahren", less: "Weniger anzeigen", clear: "Lokale Angaben löschen", close: "Hinweis schließen", reopen: "Lokale Angaben",
      detailsTitle: "Was wird gespeichert?",
      details: [
        "Nicht gesendete Vormerkungen bleiben zunächst nur in diesem Browser.",
        "Sie sind keine öffentliche Stimme und werden nicht automatisch an Vote4Gov oder eDebatte gesendet.",
        "Vote4Gov verwendet dafür keine Analyse-, Werbe- oder Tracking-Cookies.",
        "Für Hosting und Sicherheit können technisch notwendige Verbindungsdaten anfallen.",
        "Du kannst die lokalen Angaben jederzeit über den Löschbutton entfernen."
      ],
      session: n => `${n} ${n === 1 ? "Vormerkung" : "Vormerkungen"} in dieser Sitzung`,
      device: n => `${n} auf diesem Gerät gemerkt`, cookies: n => `${n} Vote4Gov-${n === 1 ? "Cookie" : "Cookies"}`, publicVotes: "0 öffentliche Stimmen bei Vote4Gov"
    },
    en: {
      ...
      storageTitle: "Your information stays on this device.",
      storageLead: "Vote4Gov does not track you. Unsent notes stay here and do not count as a public vote.",
      more: "Learn more", less: "Show less", clear: "Delete local information", close: "Close notice", reopen: "Local information",
      detailsTitle: "What is stored?",
    ...
    it: {
      ...
      ai: "Creato con l’IA e verificato dalla redazione.", aiLink: "Come usiamo l’IA", storageTitle: "I tuoi dati restano su questo dispositivo.", storageLead: "Vote4Gov non ti traccia. Le note non inviate restano qui e non contano come voto pubblico.", more: "Scopri di più", less: "Mostra meno", clear: "Elimina dati locali", close: "Chiudi", reopen: "Dati locali", detailsTitle: "Cosa viene salvato?",
    ...
    pt: {
      ...
      ai: "Criado com IA e revisto pela redação.", aiLink: "Como usamos IA", storageTitle: "Os seus dados ficam neste dispositivo.", storageLead: "A Vote4Gov não o acompanha. Notas não enviadas ficam aqui e não contam como voto público.", more: "Saber mais", less: "Mostrar menos", clear: "Apagar dados locais", close: "Fechar", reopen: "Dados locais", detailsTitle: "O que é guardado?",
    ...
    nl: {
      ...
      ai: "Gemaakt met AI en gecontroleerd door de redactie.", aiLink: "Hoe we AI gebruiken", storageTitle: "Je gegevens blijven op dit apparaat.", storageLead: "Vote4Gov volgt je niet. Niet-verzonden notities blijven hier en tellen niet als openbare stem.", more: "Meer informatie", less: "Minder tonen", clear: "Lokale gegevens wissen", close: "Melding sluiten", reopen: "Lokale gegevens", detailsTitle: "Wat wordt opgeslagen?",
    ...
    pl: {
      ...
      ai: "Utworzono z pomocą AI i sprawdzono redakcyjnie.", aiLink: "Jak korzystamy z AI", storageTitle: "Twoje dane pozostają na tym urządzeniu.", storageLead: "Vote4Gov Cię nie śledzi. Niewysłane notatki pozostają tutaj i nie są głosem publicznym.", more: "Dowiedz się więcej", less: "Pokaż mniej", clear: "Usuń dane lokalne", close: "Zamknij", reopen: "Dane lokalne", detailsTitle: "Co zapisujemy?",
    ...
    uk: {
      ...
      ai: "Створено за допомогою ШІ та перевірено редакцією.", aiLink: "Як ми використовуємо ШІ", storageTitle: "Ваші дані залишаються на цьому пристрої.", storageLead: "Vote4Gov не відстежує вас. Ненадіслані нотатки залишаються тут і не є публічним голосом.", more: "Докладніше", less: "Показати менше", clear: "Видалити локальні дані", close: "Закрити", reopen: "Локальні дані", detailsTitle: "Що зберігається?",
    ...
    zh: {
      ...
      ai: "由人工智能辅助生成并经编辑审核。", aiLink: "我们如何使用人工智能", storageTitle: "你的信息只保留在这台设备上。", storageLead: "Vote4Gov 不会跟踪你。未发送的记录只保留在这里，也不算公开投票。", more: "了解更多", less: "收起", clear: "删除本地信息", close: "关闭提示", reopen: "本地信息", detailsTitle: "保存了什么？",
  ...
  const countTopics = value => value && typeof value === "object"
    ? Object.values(value).filter(item => item && (item.stance || item.remembered)).length
    : 0;
  ...
  const snapshot = () => {
    ...
    try {
      vote4govCookies = document.cookie
        ? document.cookie.split(";").map(item => item.split("=")[0]?.trim()).filter(name => name?.startsWith("vote4gov_") || name?.startsWith("v4g_")).length
        : 0;
  ...
  const applyTopline = d => {
    ...
    if (!top) return;
    const groups = top.querySelectorAll(":scope > div");
    if (groups[0]) groups[0].innerHTML = `<strong>${d.review}</strong><span>${d.issue}</span>`;
    if (groups[1]) groups[1].innerHTML = `<span>${d.sources}</span><span>${d.compare}</span><span>${d.noRecommendation}</span>`;
  };
  ...
  const applyAiBar = d => {
    ...
    if (text) text.textContent = d.ai;
    if (link) link.textContent = d.aiLink;
  };
  ...
  const applyStorage = d => {
    ...
    const title = banner.querySelector("#storage-transparency-title");
    const lead = banner.querySelector(".storage-transparency-copy > p");
    const detailsButton = banner.querySelector("[data-storage-details]");
    const clearButton = banner.querySelector("[data-storage-clear]");
    ...
    if (title) title.textContent = d.storageTitle;
    if (lead) lead.textContent = d.storageLead;
    if (detailsButton) detailsButton.textContent = detailsButton.getAttribute("aria-expanded") === "true" ? d.less : d.more;
    if (clearButton) clearButton.textContent = d.clear;
    if (dismissButton) dismissButton.textContent = d.close;

    if (checkbox) {
      checkbox.checked = true;
      dismissButton && (dismissButton.disabled = false);
    }
    if (checkLabel) checkLabel.hidden = true;

    if (detailsPanel) {
      detailsPanel.innerHTML = `<h3>${d.detailsTitle}</h3><ul>${d.details.map(item => `<li>${item}</li>`).join("")}</ul>`;
    }
    ...
    if (reopen) {
      reopen.textContent = d.reopen;
      reopen.setAttribute("aria-label", d.reopen);
    }
